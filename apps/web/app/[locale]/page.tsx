...
import CopilotDeck, { type CopilotDeckItem, type CopilotDeckSlide } from "@/components/CopilotDeck";
...
function extractProofPoints(markdown: string) {
  return markdown
    ...
    .map((line) => stripMarkdown(line.replace(/^-\s*/, "").replace(/^◆\s*/, "")))
    .filter(Boolean)
    .slice(0, 3);
}
...
function getEntityMeta(entity: EntityMetadata, locale: string) {
  if (entity.type === "experience") {
    ...
    return [company, formatDateRange(entity.start_date, entity.end_date, locale)].filter(Boolean).join(" | ");
  }

  if (entity.type === "education") {
    const institution = entity.institution?.official_name || "";
    return [institution, formatDateRange(entity.start_date, entity.end_date, locale)].filter(Boolean).join(" | ");
  }

  if (entity.type === "venture") {
    return [entity.company_name, formatDateRange(entity.start_date, entity.end_date, locale)].filter(Boolean).join(" | ");
  }
...
function getCombinedDateRange(items: LoadedEntity[], locale: string) {
  const starts = items.map((item) => item.entity.start_date).filter((value): value is string => Boolean(value)).sort();
  const ends = items
    .map((item) => {
      const ent = item.entity;
      if (ent.type === "experience" || ent.type === "education" || ent.type === "venture") {
        return ent.end_date;
      }
      return null;
    })
...
function buildExperienceDeckItems(locale: string, experience: LoadedEntity[]) {
  ...
  const capgeminiItems = experience.filter((item) => capgeminiExperienceIds.has(item.entity.id));
  ...
  const primary =
    capgeminiItems.find((item) => item.entity.id === "experience.capgemini-apac-delivery-gpo") ||
    capgeminiItems[0];
  const otherItems = experience.filter((item) => !capgeminiExperienceIds.has(item.entity.id));
  ...
  return [combinedCapgemini, ...otherItems.map((item) => entityToDeckItem(locale, item))];
}
...
async function loadTypedEntities(type: EntityType, locale: string) {
  ...
  return loaded.filter((item): item is LoadedEntity => Boolean(item)).sort(sortNewestFirst);
}
...
export default async function HomePage({ params }: PageProps) {
  ...
  const localizedCredentials = credentials.map<CopilotDeckItem>((credential) => ({
    eyebrow: credential.category,
    href: credential.relatedHref ? `/${locale}${credential.relatedHref}` : `/${locale}/credentials`,
  ...
  const copy = isJa
    ? {
        ...
        profileSummary: aboutTranslation?.frontmatter.summary ||
          "シンガポールと日本のCxOに向けた、AI変革、ヘルステック創業、事業運営のプロフィール。",
        experienceTitle: "実行実績",
        experienceSummary: "APACでのP&L、プログラムガバナンス、エンジニアリングデリバリー。",
        educationTitle: "戦略を支える学び",
        educationSummary: "Stanford、MIT、静岡大学、経営大学院で培った戦略・運営・工学の基盤。",
        credentialsTitle: "資格スタック",
        credentialsSummary: "リーダーシップ、オペレーション、応用AI、IoT、ブロックチェーン、データサイエンス。",
        venturesTitle: "創業活動",
        venturesSummary: "ヘルスインテリジェンス、IoT、AR/VR、デジタルエンジニアリングの事業構築。",
        insightsTitle: "AI実行システム",
        insightsSummary: "フレームワーク、ガイド、コマンドセンターで構成するCxO向け実行モデル。",
        facts: [
    ...
    : {
        ...
        experienceSummary: "P&L, program governance, and engineering delivery across APAC.",
        educationTitle: "Strategic Education",
        educationSummary: "Stanford, MIT, Shizuoka, and management foundations connecting strategy, operations, and engineering.",
        credentialsTitle: "Credential Stack",
        credentialsSummary: "Leadership, operations, applied AI, IoT, blockchain, and data science credentials.",
        venturesTitle: "Founder Work",
        venturesSummary: "Health intelligence, IoT, AR/VR, and digital engineering ventures.",
        insightsTitle: "AI Transformation System",
        insightsSummary: "A framework, guide, and command center for board-to-delivery execution.",
        facts: [
  ...
  const primaryExperience = experience.find((item) => item.entity.id === "experience.capgemini-apac-delivery-gpo") || experience[0];
  const primaryVenture = ventures.find((item) => item.entity.id === "venture.nuvear") || ventures[0];
  const primaryEducation = education.find((item) => item.entity.id === "education.stanford-executive-program") || education[0];
  ...
  const profileItems: CopilotDeckItem[] = [
    ...
    },
    {
      ...
      href: primaryExperience ? getEntityHref(locale, primaryExperience.entity) : `/${locale}#experience`,
      id: "profile.enterprise-scale",
      proofPoints: isJa ? ["APACデリバリー", "日本市場", "デジタルエンジニアリング"] : ["APAC delivery", "Japan market", "Digital engineering"],
      summary: primaryExperience?.translation.frontmatter.summary ||
        (isJa ? "APACと日本でのエンタープライズ規模のデリバリー経験。" : "Enterprise-scale delivery experience across APAC and Japan."),
    ...
    },
    {
      ...
      href: primaryVenture ? getEntityHref(locale, primaryVenture.entity) : `/${locale}#ventures`,
      id: "profile.founder-dna",
      proofPoints: isJa ? ["Innuir", "HealthKitSync", "AAGNAA"] : ["Innuir", "HealthKitSync", "AAGNAA"],
      summary: primaryVenture?.translation.frontmatter.summary ||
        (isJa ? "ヘルステックと応用AIを軸にした創業活動。" : "Founder-led work in HealthTech and applied AI."),
      title: isJa ? "ヘルスインテリジェンス創業" : "Health Intelligence Founder",
      visualKind: "venture",
      visualLabel: "Innuir",
    },
    {
      eyebrow: isJa ? "学術" : "Scholarship",
      href: primaryEducation ? getEntityHref(locale, primaryEducation.entity) : `/${locale}#education`,
      id: "profile.academic-foundation",
      proofPoints: isJa ? ["Stanford GSB", "MIT", "Shizuoka University"] : ["Stanford GSB", "MIT", "Shizuoka University"],
      summary: primaryEducation?.translation.frontmatter.summary ||
        (isJa ? "戦略、オペレーション、工学をつなぐ学術的基盤。" : "Academic foundations connecting strategy, operations, and engineering."),
    ...
    },
    {
      ...
      proofPoints: localizedCredentials.slice(0, 3).map((credential) => credential.title),
  ...
  const insightItems: CopilotDeckItem[] = [
    {
      ...
      proofPoints: isJa ? ["6つの柱", "成熟度", "実行モデル"] : ["Six pillars", "Maturity model", "Execution model"],
      summary: isJa
    ...
    },
    {
      ...
      proofPoints: isJa ? ["アーキテクチャ", "展開", "ガバナンス"] : ["Architecture", "Deployment", "Governance"],
      summary: isJa
    ...
    },
    {
      ...
      proofPoints: isJa ? ["ポートフォリオ", "リスク", "ガバナンス"] : ["Portfolio", "Risk", "Governance"],
      summary: isJa
  ...
  return (
    <>
      ...
      <CopilotDeck
        contactEmail="raj@innuir.com"
        footerCopy={copy.footer}
        locale={locale}
        slides={slides}
        spotlightFacts={copy.facts}
      />
    </>
  );
}
